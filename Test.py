...
def my_prewitt(image_path):
	...
	for i in range(1,im.shape[0]-1):	
		for j in range(1,im.shape[1]-1):		
			A = abs(im.item(i-1,j-1)+im.item(i-1,j)+im.item(i-1,j+1)-im.item(i+1,j+1)-im.item(i+1,j)-im.item(i+1,j-1))
			B = abs(im.item(i-1,j+1)+im.item(i,j+1)+im.item(i+1,j+1)-im.item(i+1,j-1)-im.item(i-1,j+1)-im.item(i,j+1))
			mag = (A*A + B*B)**(.5)
...
def my_full_laplacian(image_path):
	...
	for i in range(1,im.shape[0]-1):	
		for j in range(1,im.shape[1]-1):
			A = abs(-8*im.item(i,j)+im.item(i,j+1)+im.item(i+1,j)+im.item(i-1,j)+im.item(i,j-1)+im.item(i-1,j+1)+im.item(i+1,j+1)+im.item(i-1,j-1)+im.item(i+1,j-1))
			#B = abs(im.item(i-1,j-1)+im.item(i,j-1)+im.item(i-1,j)-im.item(i+1,j+1)-im.item(i,j+1)-im.item(i+1,j))
			#mag = (A*A + B*B)**(.5)
			if(A<0):
				temp.itemset((i,j),0)
...
def trying_pyramid(image_path):
	k=0
	A = ["LowerPyramid0.jpg","LowerPyramid1.jpg","LowerPyramid2.jpg","LowerPyramid3.jpg","LowerPyramid4.jpg","LowerPyramid5.jpg"]
	while(k<6):
		im = cv2.imread(image_path,0)
...
def first_Order(image_path):
	...
	for i in range(1,im.shape[0]*2):
		...
		for j in range(1,im.shape[1]*2):
			...
			new_value = ((temp1.item(i-1,j)//2)+(temp1.item(i+1,j)//2)+(temp1.item(i,j-1)//2)+(temp1.item(i,j+1)//2)+(temp1.item(i-1,j-1)//4)+(temp1.item(i-1,j+1)//4)+(temp1.item(i+1,j-1)//4)+(temp1.item(i+1,j+1)//4)+(temp1.item(i,j)))
			if(i<10 and j<10):
